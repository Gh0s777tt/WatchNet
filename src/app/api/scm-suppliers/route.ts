...
const SUPPLIERS = [
  // Semiconduttori & Elettronica (Taiwan, Corea, Giappone)
  ...
  // Automotive & Macchinari (Europa, Messico)
...
export async function GET() {
  ...
  // Approssimazione rapida distanza (km)
  ...
  try {
    ...
    if (eqRes.ok) {
      ...
      dynamicSuppliers.forEach(sup => {
        const nearbyEq = earthquakes.filter((eq: any) => {
          ...
          return getDistanceKm(sup.lat, sup.lng, lat, lng) < 150; // zona impatto 150km
        });
    ...
    // 2. Recupera Incendi Attivi (proxy mock NASA FIRMS da locale o diretto)
    // Per prestazioni, recuperiamo dall'endpoint incendi locale poiché aggrega già FIRMS
    const fireRes = await fetch('http://127.0.0.1:3000/api/fires', { signal: AbortSignal.timeout(5000) });
    if (fireRes.ok) {
      const fireData = await fireRes.json();
      const fires = fireData.data || [];
      dynamicSuppliers.forEach(sup => {
        const nearbyFires = fires.filter((f: any) => getDistanceKm(sup.lat, sup.lng, f.lat, f.lng) < 50); // zona incendio 50km
    ...
    // 3. Recupera Zone di Conflitto (GDELT)
