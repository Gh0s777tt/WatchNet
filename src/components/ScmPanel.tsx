    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7, duration: 0.6 }} className="glass-panel p-3 pointer-events-auto mt-3 border border-[#00BCD4]/30" style={{ background: 'rgba(0, 188, 212, 0.05)' }}>
      <button onClick={() => setExpanded(!expanded)} className="flex items-center justify-between w-full mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="hud-text text-[12px] text-[var(--text-primary)]">COMANDO RISCHIO SCM</span>
          {totalRisks > 0 && (
            <span className="gotham-tag gotham-tag--critical" style={{ fontSize: '7px', padding: '1px 4px' }}>{totalRisks} ALLERTE</span>
          )}
        </div>
        <div className="flex items-center gap-2">
            ...
            <div className="space-y-3 mt-2 overflow-y-auto styled-scrollbar pr-1 flex-1 pb-4">
              ...
              {marketAlerts.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <span className="text-[11px] font-mono text-[#FF9500] tracking-widest font-bold">ALLERTE IMPATTO MERCATO</span>
                  </div>
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[11px] font-mono text-[var(--text-muted)] tracking-widest">FORNITORI CRITICI</span>
                </div>
                {criticalSuppliers.length === 0 ? (
                  <div className="text-[11px] font-mono text-[#00E676] px-2">✓ Tutti i nodi Tier 1/2 monitorati operativi.</div>
                ) : (
                  <div className="space-y-1">
                      ...
                      return (
                        <div key={i} className="px-2 py-1.5 rounded border border-[#FF1744]/40 bg-[#FF1744]/10">
                          <div className="flex justify-between items-start mb-1">
                            ...
                            <span className="text-[12px] font-mono text-[#FF1744]/80">{s.city}</span>
                          </div>
                          <div className="text-[12px] font-mono text-[#E8E6E0]">{threats.join(' • ')}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[11px] font-mono text-[var(--text-muted)] tracking-widest">NODI CONGESTIONATI</span>
                </div>
                {(congestedPorts.length === 0 && riskyChokes.length === 0) ? (
                  <div className="text-[11px] font-mono text-[#00E676] px-2">✓ Flusso marittimo globale ottimale.</div>
                ) : (
                  <div className="space-y-1">
                    {riskyChokes.map((c: any, i: number) => (
                      <div key={`c-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: c.risk === 'CRITICAL' ? '#FF1744' : '#FF9500' }}>
                        <div className="flex justify-between items-center mb-0.5">
                          ...
                          <span className="text-[12px] font-mono font-bold px-1 rounded" style={{ background: c.risk === 'CRITICAL' ? '#FF1744' : '#FF9500', color: '#000' }}>{c.risk}</span>
                        </div>
                        <div className="text-[12px] font-mono text-[#aaa]">{c.traffic}</div>
                      </div>
                    ))}
                    {congestedPorts.map((p: any, i: number) => (
                      <div key={`p-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: p.congestion === 'SEVERE' ? '#FF1744' : '#FF9500' }}>
                        <div className="flex justify-between items-center mb-0.5">
                          ...
                          <span className="text-[12px] font-mono font-bold px-1 rounded" style={{ background: p.congestion === 'SEVERE' ? '#FF1744' : '#FF9500', color: '#000' }}>{p.congestion}</span>
                        </div>
                        <div className="flex justify-between items-center text-[12px] font-mono text-[#aaa]">
                          <span>SOSTA: <span className="text-[#fff]">{p.dwell_time}</span></span>
