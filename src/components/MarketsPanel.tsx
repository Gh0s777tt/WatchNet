...
const SECTIONS = [
  { key: 'indices', label: 'INDICI', icon: LineChart },
  { key: 'stocks', label: 'DIFESA', icon: Shield },
  { key: 'oil', label: 'ENERGIA', icon: Droplets },
  { key: 'commodities', label: 'MATERIE PRIME', icon: Gem },
  ...
  return (
    <div className="flex items-center justify-between py-1.5 px-2 rounded hover:bg-[var(--hover-accent)] transition-colors">
      ...
      <div className="flex items-center gap-2">
        <span className="text-[13px] font-mono font-bold text-[var(--text-primary)] tabular-nums">
          {d.price >= 1000 ? `${(d.price / 1000).toFixed(1)}K` : d.price?.toFixed(2)}
        </span>
        <span className={`text-[13px] font-mono font-bold flex items-center gap-0.5 ${d.up ? 'text-[var(--alert-green)]' : 'text-[var(--alert-red)]'}`}>
...
export default function MarketsPanel({ data, spaceWeather }: MarketsPanelProps) {
  ...
  // Assicura che il portal sia renderizzato solo lato client
    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6, duration: 0.6 }} className={`glass-panel p-3 pointer-events-auto transition-all duration-300 flex flex-col ${maximized ? 'fixed inset-4 z-[9999] bg-[#0a0a09]/95 backdrop-blur-3xl' : ''}`}>
      <button onClick={() => setExpanded(!expanded)} className="flex items-center justify-between w-full mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="hud-text text-[12px] text-[var(--text-primary)]">MERCATI & INTELLIGENCE</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <div onClick={(e) => { e.stopPropagation(); setMaximized(!maximized); if (!expanded && !maximized) setExpanded(true); }} className="hover:text-white transition-colors cursor-pointer" title={maximized ? "Ripristina" : "Ingrandisci"} role="button" tabIndex={0}>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }}>
              ...
              <div className="mb-2 p-2 rounded-lg border" style={{ borderColor: `${spaceWeather.storm_color}33`, background: `${spaceWeather.storm_color}08` }}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[12px] font-mono tracking-widest text-[var(--text-muted)]">METEO SPAZIALE</span>
                  </div>
                ...
                {spaceWeather.solar_flares?.length > 0 && (
                  <div className="mt-1 text-[12px] font-mono text-[var(--text-muted)]">
                    Ultimo bagliore: {spaceWeather.solar_flares[0].class}
                  </div>
                )}
              </div>
            )}
              ...
              {SECTIONS.map(s => {
                ...
                return (
                  <button key={s.key} onClick={() => setActiveSection(s.key)}
                    className={`flex items-center gap-1 px-2.5 py-1.5 rounded text-[13px] font-mono tracking-wider whitespace-nowrap transition-all ${activeSection === s.key ? 'bg-[var(--hover-accent)] text-[var(--gold-primary)] border border-[var(--border-primary)]' : 'text-[var(--text-muted)] hover:text-[var(--text-secondary)] border border-transparent'}`}>
            ...
            {markets.scm_alerts && markets.scm_alerts.length > 0 && (
              <div className="mb-2 space-y-1">
                {markets.scm_alerts.map((alert: string, i: number) => (
                  <div key={i} className="px-2 py-1.5 rounded border border-[#FF9500] bg-[#FF9500]/10 text-[#FF9500] text-[13px] font-mono leading-tight shadow-[0_0_8px_rgba(255,149,0,0.15)]">
                    {alert}
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-0.5 overflow-y-auto styled-scrollbar mt-2">
              ...
              {(!markets[activeSection] || Object.keys(markets[activeSection]).length === 0) && (
                <div className="text-center py-3 text-[12px] font-mono text-[var(--text-muted)]">Caricamento {activeSection}...</div>
              )}
            </div>
          </motion.div>
        )}
